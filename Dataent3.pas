...
procedure VeriGir(AlanNo: integer; BasHarf: char); 
var
  Girdi: string; 
  X, Y: integer;
  Ch: char;
...
begin
  ...
  Pos := Length(Girdi) + 1; 
  CikisVeriGir := False; 
  if (AlanNo = 1) and (BasHarf in ['0'..'9']) and (Length(Girdi) < MaxUzunluk) then
  begin
    Insert(BasHarf, Girdi, Pos);
    Inc(Pos); 
  end
  ...
  else if (AlanNo = 7) and (UpCase(BasHarf) in ['E', 'K']) then
 
  begin
 
    Girdi := UpCase(BasHarf);
